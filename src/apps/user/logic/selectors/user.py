...
from src.apps.user.logic.queries.user import users_q__by_email, users_q__by_pk
...
async def user__find_by_pk(*, pk: int, session: AsyncSession | None = None) -> User | None:
    query = users_q__by_pk(pk=pk)
    async with optional_session_generator(session=session) as async_session:
        scalars = await async_session.scalars(query)
        return scalars.one_or_none()
